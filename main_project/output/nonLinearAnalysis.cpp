...
string runNonLinearTransience(Circuit& c, double t, VectorXd& interpolX1, VectorXd& interpolX2, double& interpolT1, double& interpolT2,double& printTime,vector<float>& interpolI1, vector<float>& interpolI2){
    ...
    double maxDynamicTimeStep = c.getMaxTimeStep();
    ...
    do{
        ...
        if(dynamicTimeStep < minDynamicTimeStep && t != c.getSimulationTime()){
    ...
    if(t<=printTime || abs(printTime)<0.000000001){
        ...
        for(const auto &comp : components){
            if(typeid(*comp)!=typeid(Mosfet) && typeid(*comp) !=typeid(BJT)){     
                ...
                interpolI1.push_back((float)stod(res1)); //This line is only required for the CSVTester (Use the commented out one above for normal simulations!)
        ...
        if(t!=printTime){
            for(const auto &comp : components){
                if(typeid(*comp)!=typeid(Mosfet) && typeid(*comp) !=typeid(BJT)){            
                    ...
                    interpolI2.push_back((float)stod(res1)); //This line is only required for the CSVTester (Use the commented out one above for normal simulations!)
